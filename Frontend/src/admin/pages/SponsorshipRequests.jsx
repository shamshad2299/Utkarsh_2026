...
const SponsorshipRequests = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await sponsorshipService.getAll(params);
      setItems(res?.data || []);
    } catch (err) {
      alert(
        err?.response?.data?.message || "Failed to load sponsorship requests"
      );
    } finally {
      setLoading(false);
    }
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((x) => {
      return (
        ...
        x.sponsorshipCategory?.toLowerCase().includes(q)
      );
    });
  }, [items, search]);
  ...
  const deleteItem = async (id) => {
    ...
    setDeletingId(id);
    try {
      await sponsorshipService.delete(id);
      await fetchData();
    } catch (err) {
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            View and manage all sponsorship applications
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl border p-4 mb-6">
        <div className="flex flex-col md:flex-row gap-3">
          <input
            ...
            placeholder="Search by business name, email, owner, phone, category..."
            value={search}
                ...
                {filtered.map((x) => (
                  <tr key={x._id} className="border-t">
                    ...
                    <td className="px-4 py-3 text-gray-900">
                      {x.sponsorshipCategory}
                    </td>
                    <td className="px-4 py-3 text-gray-900">₹{x.amount}</td>
