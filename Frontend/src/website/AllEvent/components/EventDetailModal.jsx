...
import { useEvent, useCategories, useSubCategories } from '../../../features/eventsAPI';
...
const EventBadge = React.memo(({ children, className }) => (
  <div className={`inline-flex items-center gap-1.5 sm:gap-2 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full backdrop-blur-sm border border-white/30 ${className}`}>
    {children}
  </div>
));
    ...
    <div className="bg-white/80 backdrop-blur-sm rounded-xl border-2 border-black/30 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3 sm:gap-4">
          <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-r ${getColorClasses().split(' ')[0]} ${getColorClasses().split(' ')[1]} flex items-center justify-center`}>
            <span className={`${getColorClasses().split(' ')[2]} font-bold text-base sm:text-lg milonga`}>
              {icon}
            </span>
          </div>
...
const EventDetailModal = ({
  ...
  const { data: selectedEvent, isLoading: eventLoading, error: eventError } = useEvent(eventId, {
  ...
  const categoryRules = useMemo(() => {
    if (currentCategory?.rules && Array.isArray(currentCategory.rules) && currentCategory.rules.length > 0) {
      return currentCategory.rules;
    }
    // Default category rules if none provided
    return [
      "Follow the event guidelines and code of conduct.",
      "Respect the judges, organizers, and fellow participants.",
  ...
  const subCategoryRules = useMemo(() => {
    if (currentSubCategory?.rules && Array.isArray(currentSubCategory.rules) && currentSubCategory.rules.length > 0) {
      return currentSubCategory.rules;
    }
    // Default subcategory rules if none provided
    return [
      "Event-specific guidelines apply.",
      "Follow the format and structure defined for this category.",
      "Technical requirements must be fulfilled before the event.",
      "Time limits will be strictly enforced.",
      "Originality and creativity are encouraged.",
      "Props and equipment must be pre-approved."
    ];
  }, [currentSubCategory]);
  ...
  const eventSpecificRules = useMemo(() => {
    ...
    // Add event description as first rule if it exists
    if (selectedEvent?.description) {
      rules.push(`📋 Event Description: ${selectedEvent.description}`);
    }
    
    // Add event-specific rules from subcategory
    if (subCategoryRules.length > 0) {
      rules.push(...subCategoryRules.map(rule => `🎯 ${rule}`));
    }
    
    // If no rules at all, add placeholder
    if (rules.length === 0) {
  ...
  const isRegistrationOpen = useMemo(() => 
    selectedEvent ? new Date() <= new Date(selectedEvent.registrationDeadline) : false,
    [selectedEvent]
  );
  
  // Check event capacity
  const isFull = useMemo(() => 
    selectedEvent ? selectedEvent.currentParticipants >= selectedEvent.capacity : false,
    [selectedEvent]
  );
  ...
  const handleEnrollSubmit = useCallback(async (event, teamId = null) => {
    try {
      setIsEnrolling(true);
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
        <div className="bg-white rounded-xl p-8 flex flex-col items-center">
          ...
          <p className="text-[#2b123f] font-medium milonga">No event selected...</p>
        </div>
      </div>
    );
  }
  ...
  if (eventLoading) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
        <div className="bg-white rounded-xl p-8 flex flex-col items-center">
          <div className="w-12 h-12 border-4 border-[#4b1b7a] border-t-transparent rounded-full animate-spin mb-4"></div>
          <p className="text-[#2b123f] font-medium milonga">Loading event details...</p>
        </div>
      </div>
    );
  }
